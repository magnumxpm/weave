...
def arrow(
    ...
    *,
    ...
    if halo:
        out += (
            f'<path d="{path}" fill="none" stroke="#FFFFFF" stroke-width="5" '
    ...
    if label and label_pos is not None:
        ...
        out += (
            group + f'<rect x="{anchor_x}" y="{ly - 9}" width="{w}" height="13" rx="3" '
...
def system_map() -> str:
    ...
    s += box(
        ...
        accent=True,
        sub="the only broad delegation",
    )

    s += panel(
        44, 412, 572, 112, "Vertex AI Agent Engine \u2014 no delegation, ever", fill="#FFFFFF"
    )
    s += box(
        60, 444, 260, 60, ["Pipeline engine", "extraction + enrichment", "two phases, one call"]
    )
    s += box(340, 444, 260, 60, ["Copilot engine", "12 principal-scoped tools", "read + lifecycle"])
    ...
    s += arrow(
        [(128, 286), (128, 348), (160, 348)], blue=True, label="OIDC push", label_pos=(128, 322)
    )
    ...
    s += arrow(
        [(300, 444), (300, 398), (352, 398), (352, 378)],
        label="context broker",
        label_pos=(326, 394),
    )
    s += arrow(
        [(430, 378), (430, 444)], blue=True, label="ask, as this principal", label_pos=(430, 414)
    )
    ...
    s += arrow(
        [(500, 348), (632, 348), (632, 124), (580, 124), (580, 100)],
        blue=True,
        halo=True,
        label="delegated reads, as the owner",
        label_pos=(628, 246),
        label_rotate=-90,
    )
    s += arrow(
        [(160, 332), (28, 332), (28, 124), (120, 124), (120, 100)],
        blue=True,
        halo=True,
        label="card \u00b7 reply",
        label_pos=(76, 133),
    ...
    s += arrow(
        [(532, 184), (532, 140), (380, 140), (380, 100)],
        halo=True,
        label="per-user subscriptions",
        label_pos=(456, 136),
    )

    s += (
        f'<line x1="44" y1="600" x2="120" y2="600" stroke="{BLUE}" stroke-width="1.4"/>'
...
def capability_boundary() -> str:
    ...
    for x0, rows in ((26, left), (370, right)):
        ...
        for present, label in rows:
            mark, colour, bg = (
                ("\u2713", GREEN, "#EAF7F0") if present else ("\u2715", "#C8402F", "#FDEDEA")
            )
            s += f'<circle cx="{x0 + 9}" cy="{y - 4}" r="9" fill="{bg}"/>'
            s += (
                f'<text x="{x0 + 9}" y="{y}" text-anchor="middle" font-size="10.5" '
    ...
    s += (
        f'<line x1="330" y1="26" x2="330" y2="218" stroke="{BLUE}" stroke-width="1.4" '
        f'stroke-dasharray="5 5"/>'
    )
    s += (
        f'<g transform="rotate(-90 330 122)">'
    ...
    for cx, small, big in (
        ...
        s += (
            f'<text x="{cx}" y="188" text-anchor="middle" font-size="10" fill="{MUTED}">'
            f"{esc(small)}</text>"
        )
        s += (
            f'<text x="{cx}" y="210" text-anchor="middle" font-size="11" font-weight="700" '
...
def urgency_ladder() -> str:
    ...
    for i, (name, colour, note) in enumerate(groups):
        ...
        s += (
            f'<text x="{x + w / 2:.1f}" y="136" text-anchor="middle" font-size="8.5" '
            f'fill="{MUTED}">{esc(note)}</text>'
        )
        x += w + 6
    s += f'<line x1="14" y1="150" x2="{W - 14}" y2="150" stroke="{BORDER}"/>'
    s += (
        f'<text x="14" y="170" font-size="10.5" font-weight="600" fill="{INK}">'
        "Declaration order is display order</text>"
    )
    s += (
        f'<text x="{W - 14}" y="170" text-anchor="end" font-size="10.5" fill="{MUTED}">'
...
def context_framework() -> str:
    ...
    s += box(14, 128, 150, 62, ["search_related_context", "the one context tool"], accent=True)
    ...
    s += box(
        ...
        110,
        74,
        ["visible_to", "array_contains", "in the query"],
        fill="#F2FBF6",
        stroke="#BEE6D0",
    )
    s += box(
        ...
        110,
        74,
        ["POST", "/context/search", "read as the owner"],
        fill="#F2FBF6",
        stroke="#BEE6D0",
    )
    ...
    s += (
        f'<text x="591" y="118" text-anchor="middle" font-size="9.5" fill="{MUTED}">'
        f"the database refuses</text>"
        f'<text x="591" y="130" text-anchor="middle" font-size="9.5" fill="{MUTED}">'
        f"what it cannot show</text>"
    )
    s += (
        f'<text x="591" y="310" text-anchor="middle" font-size="9.5" fill="{MUTED}">'
    ...
    s += f'<rect x="14" y="216" width="306" height="118" rx="10" fill="#FFFFFF" stroke="{BORDER}"/>'
    s += (
        f'<text x="30" y="240" font-size="10" font-weight="600" letter-spacing="0.9" '
