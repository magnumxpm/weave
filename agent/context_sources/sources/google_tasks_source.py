"""Owner-scoped open Google Tasks context via the ingestion broker."""
...
@register_source("google_tasks", AuthMode.USER_CONTEXT)
class GoogleTasksSource(ContextSource):
    def __init__(
        self,
    ...
    def search(self, query: str, principal: SearchPrincipal, limit: int = 5) -> list[ContextMatch]:
        if not self._base_url or not self._audience:
            logger.warning("google_tasks source disabled: broker not configured")
            return []
